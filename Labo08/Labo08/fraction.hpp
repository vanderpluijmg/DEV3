...
// implémentation de méthodes inline
    ...
    inline std::tuple<Sign, unsigned, unsigned, unsigned> Fraction::unit_form() const {
        return std::tuple<Sign, unsigned, unsigned, unsigned> {sign(), numerator()/denominator(),
                                                               numerator(), denominator()};
    }


    inline Fraction::operator double() const {
        return g54786::Sign::MINUS == sign()?
            -static_cast<double>((numerator() / denominator())) : static_cast<double>(numerator()/denominator());
    }

// implémentation de fonctions inline

        inline Fraction abs(const Fraction &fraction) {
            return Fraction(g54786::Sign::PLUS, fraction.numerator(), fraction.denominator());
        }
        ...
        inline std::ostream &operator<<(std::ostream &out, const Fraction &fraction) {
            ...
            return out << fraction.to_string() << std::endl;
        }
        ...
        inline bool operator>(const Fraction &lhs, const Fraction &rhs) {
        if (lhs.sign() == g54786::Sign::MINUS)
            return false;
        else if (rhs.sign() == g54786::Sign::MINUS)
            return true;
        ...
        inline Fraction operator+(const Fraction &fraction) {
            return Fraction(fraction.sign(), fraction.numerator());
        }

        inline Fraction operator-(const Fraction &fraction) {
            return fraction.sign() == g54786::Sign::MINUS?
            Fraction(Sign::PLUS, fraction.numerator(), fraction.denominator()) :
            Fraction(Sign::MINUS, fraction.numerator(), fraction.denominator());
