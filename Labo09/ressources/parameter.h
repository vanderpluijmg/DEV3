 ...
 * \file parameter.h
 *
 * \brief Définition de la classe nvs::lotto::Parameter ainsi que
...
// à partir de c++17, il est plus aisé qu'avant de définir des
// espaces de nom imbriqués :
...
class Parameter
{
  ...
  public:
     ...
     */
    constexpr explicit Parameter(unsigned length = LENGTH_DEFAULT_,
     ...
     */
    constexpr unsigned minimum() const;
     ...
     */
    constexpr unsigned maximum() const;
...
constexpr Parameter::Parameter(unsigned length, unsigned maximum,
                               unsigned minimum) :
    minimum_ { minimum },
    maximum_ { maximum < minimum_ ?
               throw std::invalid_argument { "maximum < minimum_" } :
               maximum },
    length_ { length == 0 ?
              throw std::invalid_argument { "length == 0" } :
              maximum_ - minimum_ + 1 < length ?
              throw std::invalid_argument { "maximum_ - minimum_ + 1 "
                                            "< length" } :
              length}
{ }
